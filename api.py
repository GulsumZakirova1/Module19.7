...
class PetFriends:
    ...
    def get_api_key_with_invalid_password(self, email: str, password: str) -> json:
        """Метод делает запрос с невалидным паролем и возвращает статус не равный 200
    ...
    def create_pet_with_empty_value_name(self, auth_key: json, name: str, animal_type: str, age: str) -> json:
        """Метод отправляет (постит) на сервер данные о добавляемом питомце без фото и возвращает статус
                        запроса на сервер и результат в формате JSON с данными добавленного питомца"""
    ...
    def get_api_key_with_mistaken_email(self, email: str, password: str) -> json:
        """Метод делает запрос с ошибкой email и с валидным паролем и возвращает статус не
        равный 200 и в результате  не содержится key"""
